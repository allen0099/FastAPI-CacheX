...
class AsyncValkeyCacheBackend(BaseCacheBackend):
    """Async Valkey cache backend implementation."""

    client: "AsyncValkey[str]"
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Initialize async Valkey cache backend.
        ...
        try:
            ...
            raise CacheXError(
                "valkey is not installed. Please install it with 'pip install valkey'"
            )

        self.client = AsyncValkey(
            host=host,
            port=port,
            password=password,
            db=db,
