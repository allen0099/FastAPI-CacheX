...
The library has three independent subsystems:
...
- Cache values are stored as `ETagContent(etag, content, media_type)` namedtuple (defined in `types.py`).

**2. Session Management (`fastapi_cachex/session/`)**
- Optional subsystem, activated via `SessionMiddleware` and `SessionManagerProxy`.
- `SessionManager` handles create/get/update/delete/invalidate/regenerate operations. It stores `Session` Pydantic models serialized as JSON, wrapped in `ETagContent` for backend compatibility.
...
**3. State Management (`fastapi_cachex/state/`)**
...
- `MemcachedBackend` (`backends/memcached.py`): Does not support `clear_pattern` or `get_all_keys`. Requires `pymemcache` extra.
